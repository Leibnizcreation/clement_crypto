...
class Dashboard extends Component {
  ...
  convert() {
    ...
    axios.post(`${apiUrl}/api/finance/convert`, {
    ...
    if (histories.length > 0) {
      History = histories.map((history, i) => (
        <tr key={i}>
          ...
          <td>{history.type}</td>
          <td>{history.site ? history.site : ''}</td>
  ...
  cashOut() {
    ...
    axios.post(`${apiUrl}/api/finance/payout`, {
  ...
  changePassword() {
    ...
    axios.post(`${apiUrl}/api/auth/change-password`, { newPass, currentPass })
      .then(() => {
        swal({
          title: 'Success',
  ...
  generateQR() {
    axios.post(`${apiUrl}/api/auth/generate-2fa`).then(({ data }) => {
      this.setState({
        qrImg: data.dataURL,
  ...
  confirmToken() {
    ...
    axios.post(`${apiUrl}/api/auth/verify-2fa`, { token: this.state.token }).then(({ data }) => {
  ...
  disableTfa() {
    swal({
      ...
      if (result.value) {
        axios.post(`${apiUrl}/api/auth/disable-2fa`).then(({ data }) => {
          window.localStorage.setItem('jwtTokenBTCGrinders', data.token);
          swal(
            'Success!',
