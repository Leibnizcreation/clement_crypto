...
router.post('/earnings', authUserMiddleware, (req, res) => {
  ...
  const {
    siteName, email, siteType, amount, rewardType
  } = req.body;
  ...
  Earning.findOne({ email, type }).then((earn) => {
    if (earn) {
      ...
      Earning.update({ email, type }, { amount: newAmount }).then(() => {
        console.log(siteName, siteType, email);
        Site.findOne({ email, site_type: siteType, site_name: siteName })
          .then((site) => {
            ...
            if (!site) {
              ...
              const newReward = parseFloat(site[rewardType]) + parseFloat(amount);
              Site.update({ email, siteType, siteName }, { [rewardType]: newReward })
                .then(() => {
                  History.create({
                    id,
                    email,
